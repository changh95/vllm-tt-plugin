...
| `tt_adaptive_block_ragged` | `False` | Batched adaptive block models only (requires `tt_adaptive_block_batched`): a decode-only block step returns a rectangular int32 `[num_reqs, W]` tensor whose row `i` holds `1 <= n_i <= W` real token ids followed by `-1` padding; the runner counts `n_i` as the non-negative ids, strips the padding and appends only those tokens, and the scheduler accepts `1..W` per request while still consuming the whole `W` placeholder reservation (computed tokens advance by `n_i`, the per-step reservation and KV lookahead stay `W`-wide). Prefill anchors stay width-1 |
